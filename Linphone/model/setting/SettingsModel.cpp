...
QVariantList SettingsModel::getAiAgents() const {
	mustBeInLinphoneThread(log().arg(Q_FUNC_INFO));
	QVariantList agents;
	auto sections = mConfig->getSectionsNamesList();
	for (auto section : sections) {
		QString sectionStr = Utils::coreStringToAppString(section);
		if (sectionStr.startsWith("ai_agent_")) {
			QVariantMap agent;
			agent["name"] = Utils::coreStringToAppString(mConfig->getString(section, "name", ""));
			agent["provider"] = Utils::coreStringToAppString(mConfig->getString(section, "provider", "gemini"));
			agent["apiKey"] = Utils::coreStringToAppString(mConfig->getString(section, "api_key", ""));
			agent["model"] =
			    Utils::coreStringToAppString(mConfig->getString(section, "model", "gemini-2.0-flash-live-001"));
			agent["voice"] = Utils::coreStringToAppString(mConfig->getString(section, "voice", "Puck"));
			agent["language"] = Utils::coreStringToAppString(mConfig->getString(section, "language", "en-US"));
			agents << agent;
		}
	}
	return agents;
...
void SettingsModel::setAiAgents(const QVariantList &agents) {
	mustBeInLinphoneThread(log().arg(Q_FUNC_INFO));
	auto sections = mConfig->getSectionsNamesList();
	for (auto section : sections) {
		if (Utils::coreStringToAppString(section).startsWith("ai_agent_")) {
			mConfig->cleanSection(section);
		}
	}
	int count = 0;
	for (auto agent : agents) {
		auto m = agent.toMap();
		auto key = Utils::appStringToCoreString("ai_agent_" + QString::number(count++));
		mConfig->setString(key, "name", Utils::appStringToCoreString(m["name"].toString()));
		mConfig->setString(key, "provider", Utils::appStringToCoreString(m["provider"].toString()));
		mConfig->setString(key, "api_key", Utils::appStringToCoreString(m["apiKey"].toString()));
		mConfig->setString(key, "model", Utils::appStringToCoreString(m["model"].toString()));
		mConfig->setString(key, "voice", Utils::appStringToCoreString(m["voice"].toString()));
		mConfig->setString(key, "language", Utils::appStringToCoreString(m["language"].toString()));
	}
	emit aiAgentsChanged(agents);
}

QVariantList SettingsModel::getAiScenarios() const {
	mustBeInLinphoneThread(log().arg(Q_FUNC_INFO));
	QVariantList scenarios;
	auto sections = mConfig->getSectionsNamesList();
	for (auto section : sections) {
		QString sectionStr = Utils::coreStringToAppString(section);
		if (sectionStr.startsWith("ai_scenario_")) {
			QVariantMap scenario;
			scenario["name"] = Utils::coreStringToAppString(mConfig->getString(section, "name", ""));
			scenario["agentIndex"] = mConfig->getInt(section, "agent_index", 0);
			scenario["systemPrompt"] = Utils::coreStringToAppString(mConfig->getString(section, "system_prompt", ""));
			scenarios << scenario;
		}
	}
	return scenarios;
...
void SettingsModel::setAiScenarios(const QVariantList &scenarios) {
	mustBeInLinphoneThread(log().arg(Q_FUNC_INFO));
	auto sections = mConfig->getSectionsNamesList();
	for (auto section : sections) {
		if (Utils::coreStringToAppString(section).startsWith("ai_scenario_")) {
			mConfig->cleanSection(section);
		}
	}
	int count = 0;
	for (auto scenario : scenarios) {
		auto m = scenario.toMap();
		auto key = Utils::appStringToCoreString("ai_scenario_" + QString::number(count++));
		mConfig->setString(key, "name", Utils::appStringToCoreString(m["name"].toString()));
		mConfig->setInt(key, "agent_index", m["agentIndex"].toInt());
		mConfig->setString(key, "system_prompt", Utils::appStringToCoreString(m["systemPrompt"].toString()));
	}
	emit aiScenariosChanged(scenarios);
}
